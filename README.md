...
| 스킬 | 언제 쓰는가 | 하는 일 |
| --- | --- | --- |
| `init` | 새 기술로 예제를 시작할 때 | 고른 학습 활동과 수준에 맞는 예제 하나와 학습 환경을 준비 |
| `tutor` | 예제를 진행하다 모르는 것이 생기거나 찾은 결과를 확인받고 싶을 때 | 답변 단계를 정해 그 범위까지만 답변 |
...
`init`은 배우려는 계기와 마쳤을 때 할 수 있게 되고 싶은 일을 먼저 묻습니다. 그 답으로 학습 활동 후보를 2~3개 추천하고, 고른 활동과 수준으로 학습 목표를 정해 폴더를 만들기 전에 확인받습니다. 활동은 만들기에 그치지 않습니다. 문제가 심어진 앱에서 원인을 찾거나 동작을 관찰하는 활동도 고를 수 있습니다. 예제를 시작할 때 `tutor`를 따로 부르지 않습니다.

`tutor`는 예제 폴더 안에서 부릅니다. 처음 물으면 무엇을 시도했고 어디서 막혔는지 되묻고, 더 알려 달라고 할 때만 한 단계씩 넓혀 답합니다. 해결 방법은 마지막 단계에서야 나옵니다. 찾은 원인이나 관찰 결과를 `PLAN.md`의 `기록` 절에 적고 맞는지 물으면, 같은 단계 규칙으로 코드와 대조해 답합니다.
...
- 예제 폴더는 `init`을 부른 디렉터리 아래에 `<기술>-<학습 활동 요약>` 이름으로 생기고, 안에 `README.md`·`TASK.md`·`PLAN.md` 셋이 들어갑니다.
