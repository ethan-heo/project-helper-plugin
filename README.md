...
설명을 듣는 대신 직접 구현하며 배우는 스킬입니다. `init`·`tutor`·`manager` 셋으로 이루어지며, 지금은 `init`과 `tutor`가 만들어져 있습니다.

| 스킬 | 언제 쓰는가 | 하는 일 | 상태 |
| --- | --- | --- | --- |
| `init` | 새 기술로 예제를 시작할 때 | 수준에 맞는 예제 하나와 학습 환경을 준비 | 사용 가능 |
| `tutor` | 예제를 구현하다 모르는 것이 생겼을 때 | 답변 단계를 정해 그 범위까지만 답변 | 사용 가능 |
| `manager` | 마친 예제를 복습하고 다음 학습을 정할 때 | 복습 질문과 이해 수준 진단, 다음 학습 제안 | 준비 중 |
