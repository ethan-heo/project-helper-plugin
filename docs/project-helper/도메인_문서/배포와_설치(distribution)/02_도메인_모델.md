...
| 이름 | 구분 | 역할 |
| --- | --- | --- |
| 마켓플레이스 매니페스트 | 엔티티 | 이름 `helpers`, 소유자, 플러그인 목록 |
...
| 스킬 디렉터리 | 엔티티 | 설치본에 복사되는 실체. `skills/` 아래에는 스킬 네 개와 공용 참조 디렉터리 `_shared/`가 있다 |
...
| `plugins/project-helper/skills/` | 스킬 본체와 공용 참조 문서 | 양쪽 |
| `plugins/project-helper/skills/_shared/` | 여러 스킬이 함께 읽는 판정 규칙. `SKILL.md`가 없어 스킬로 등록되지 않는다 | 양쪽 |

### `.claude-plugin/marketplace.json`

| 필드 | 타입 | 제약 조건 | 설명 |
| --- | --- | --- | --- |
| `name` | string | 필수 | 마켓플레이스 이름. 현재 값은 `helpers` |
