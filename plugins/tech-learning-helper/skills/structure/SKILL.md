...
질문은 패키지의 `questions.md`에서 같은 뜻이면서 관점도 구조인 질문을 찾습니다. 같은 뜻의 질문이 실행 관점으로만 저장되어 있으면, Example Builder의 규칙으로 구조 관점 경로를 새로 만들어 `questions.md`에 더합니다. 후보를 제시할 때도 구조 관점으로 저장된 질문을 먼저 쓰고, 모자라면 새로 만듭니다.
