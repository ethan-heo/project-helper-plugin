...
2. 학습 패키지 기록 파일 가운데 가장 늦은 날짜 파일의 마지막 질문 절(`## 실험:` 절 제외)
...
실험의 질의응답은 그날 기록 파일에 `## 실험: <대상 질문>` 절로 적습니다(`record.md`). 실험이 끝나면 다음 탐색 후보 2~3개를 번호로 제시하고, 이후 질문은 `session.md`의 배정 표로 받습니다.
