...
추가 작업은 [계획에 추가 작업 반영](../_shared/session.md#계획에-추가-작업-반영)에 따라 처리한다.
...
| 2 | [`existing-docs.md`](references/planning/existing-docs.md)를 읽고 프로젝트의 기존 문서를 찾아 읽는다. 아키텍처 문서의 선행 조건을 아래 분기로 확인한다 |
...
**초안을 만들기 전에 아키텍처 문서의 결정으로 요청을 해결할 수 있는지 확인한다.**

구조 결정의 범위는 [공통 판단 기준](../_shared/architecture.md#구조-결정의-범위)을 따른다.

| 선행 상태 | 할 일 |
| --- | --- |
| 아키텍처 문서가 없다 | `create-architecture` 실행 후 계획 재개 |
| 요청이 문서의 결정 안에서 해결된다 | 관련 문서 확인과 기준 버전 기록 |
| 문서에 없는 구조 결정이 필요하다 | 아키텍처 문서 갱신 후 계획 재개 |
| 계획 설계가 문서의 결정과 어긋난다 | 설계 수정·문서 갱신 중 사용자 선택 |

아키텍처 작업을 먼저 해야 하면 [선행 실행과 계획 재개](references/planning/existing-docs.md#선행-실행과-계획-재개)를 따른다.

문서 조사와 코드 대조는 [기존 문서 조사의 예외 처리](references/planning/existing-docs.md#예외-처리)를 따른다.

문서별 역할과 상태 관리 위치는 [문서 계층](references/documents/hierarchy.md)을 따른다.

대화 중에는 초안 한 편에 기록하고, 구현 순서가 정해지면 4-1번에서 디렉터리 구조로 옮긴다.
...
계획서의 문체와 표기는 [공유 표기 기준](../_shared/writing-style.md)을 따른다.

질문·힌트·대안은 [질문과 대안](references/collaboration/questioning.md)을 따른다. 카테고리의 답변이 구현자가 추가 질문 없이 작성할 만큼 구체적이면 기록하고 다음으로 넘어간다.
...
아키텍처 결정의 인용은 [계획서에 반영하는 방식](references/planning/existing-docs.md#계획서에-반영하는-방식)을 따른다.

설계 도중 새로운 구조 결정이나 기존 결정과의 충돌을 발견하면 준비 단계의 분기로 돌아간다. 처리한 뒤 해당 설계부터 이어 간다.
...
**[경계 판정](references/planning/boundaries.md)에 따라 경계를 제안하고 사용자 동의를 받는다.**

**[문서 계층](references/documents/hierarchy.md)에 따라 위층부터 내용을 옮기고 초안을 삭제한다.**

문서 루트를 최상위 문서에 남기는 이유는 `impl-plan`이 탐색을 다시 하지 않고 같은 위치를 쓰게 하기 위해서다. 후보가 여럿이라 사용자에게 물어 정했다면, 그 경로를 프로젝트 지시사항에도 적어 두기를 함께 제안한다.
...
| --- | --- |
| 1 | 규격과 대조해 표기를 확인한다 |
| 2 | 미해결 검토 표시가 남아 있으면 표시된 결정을 사용자와 정리한다. 하나라도 남으면 확정할 수 없다 |
| 3 | `writing-style.md`의 어휘 절과 용어 규칙을 세 층 문서 전체에 대조한다 |
| 4 | 인계 점검을 한다. 항목마다 "이것만 읽고 무엇을 어디에 할지 정해지는가"를 묻는다 |
| 5 | 구현 진행률의 전체 수를 구현 항목 수로 맞춘다 |

확정 직전에는 [아키텍처 기준 버전 확정](references/planning/existing-docs.md#아키텍처-기준-버전-확정)을 따른다.
...
- 여러 계획이 따를 구조·패턴·계약은 `/create-architecture`에서 정한다. 문서가 없거나 새 구조 결정이 필요하면 같은 대화에서 선행 실행한 뒤 계획을 재개한다.
