...
| 4 | 구성 요소 | 뒤 두 절에 나오는 코드 식별자의 정체 | 표 | 실행 흐름과 의존 관계에 등장하는 식별자 중 표에 없는 것이 없다 |
| 5 | 실행 흐름 | 입력부터 결과까지의 단계 | 표 | 단계마다 파일과 함수 이름이 붙어 있고, 이 커밋으로 달라진 단계가 표시되어 있다 |
| 6 | 의존 관계 | 구성 요소 사이의 연결 | 다이어그램과 표 | mermaid 블록이 있고, 화살표마다 하위가 상위에 대해 알아야 하는 지식이 표에 적혀 있다 |
