...
| --- | --- | --- | --- | --- |
| 1 | 한 줄 요약 | 이 커밋이 무엇을 바꿨는지 한 문장 | 문장 한 줄 | 이 문장만 읽고 커밋의 대상과 변화를 말할 수 있다 |
| 2 | 변경 전 상태와 문제 | 직전 코드가 어떻게 동작했고 무엇이 막혔으며 어디서 돌았는지 | 라벨 세 줄 | 이전 동작과 문제, 실행 환경이 각각 한 문장으로 적혀 있다 |
| 3 | 변경 의도 | 왜 그렇게 바꿨는지와 그 근거 | 목록 | 의도 서술마다 근거가 된 커밋 메시지나 계획서가 함께 적혀 있다 |
| 4 | 의존 관계 | 구성 요소 사이의 연결과 구조 | 다이어그램과 표 | mermaid 블록이 있고, 화살표마다 하위가 상위에 대해 알아야 하는 지식이 표에 적혀 있다 |
| 5 | 구성 요소 | 앞뒤 두 절에 나오는 코드 식별자의 정체 | 표 | 의존 관계와 실행 흐름에 등장하는 식별자 중 표에 없는 것이 없다 |
| 6 | 실행 흐름 | 입력부터 결과까지의 단계 | 표 | 단계마다 파일과 함수 이름이 붙어 있고, 이 커밋으로 달라진 단계가 표시되어 있다 |
| 7 | 용어와 배경 | 본문에 나온 개념의 뜻 | 표 | 본문의 전문 용어 중 표에 없는 것이 없다 |
...
| 이전 코드의 실행 환경을 확인할 수 없다 | 그 자리에 확인 필요 표시를 남깁니다 |
